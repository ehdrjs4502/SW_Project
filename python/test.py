...
time.sleep(3)
new_filename = f"C:\\test\\typhoon{now.date()}.xls" # 새로운 파일 이름
...
time.sleep(1)
driver.find_element(By.CLASS_NAME,'upload-file-input').send_keys(new_filename) # 엑셀 업로드
...
queryName = f"typhoonQuery{now.date()}"

time.sleep(2)
driver.find_element(By.XPATH,xpath).send_keys(queryName) # 파일 이름 typhoonQuery+현재 날짜로 지정
...
# 호출: sql파일 수정
replace_in_file(f"C:\\test\\{queryName}.sql", "INTEGER", "BIGINT") # INTEGER -> BIGINT로 수정
replace_in_file(f"C:\\test\\{queryName}.sql", "NOT NULL", "") # NOT NULL 없애기
replace_in_file(f"C:\\test\\{queryName}.sql", "PRIMARY KEY", "") # PRIMARY KEY 없애기

#디비 연동  **CLIENT.MULTI_STATEMENTS = 멀티쿼리를 지원하는 옵션**
conn = pymysql.connect(host='127.0.0.1', user='root', password='1234', db='mydb', charset='utf8', client_flag=CLIENT.MULTI_STATEMENTS)
cursor = conn.cursor()

# 테이블 이름 변경 쿼리
#ALTER TABLE `mydb`.`mytabletyphoontest`  RENAME TO  `mydb`.`mytabletyphoontest123` ;

renameQuery = f"ALTER TABLE `mydb`.`mytabletyphoontest` RENAME TO  `mydb`.`mytabletyphoontest{now.date()}`;"

cursor.execute(renameQuery)
cursor.fetchall()
conn.commit() # 커밋

sql = open(f"C:\\test\\{queryName}.sql", encoding='utf8').read() # sql 파일 읽기
